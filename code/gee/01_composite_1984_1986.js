...
// Baseline composite: 1984-01-01 to 1986-12-31 (Landsat 5 only)
...
// Earliest possible Landsat era: Landsat 5 launched March 1984.
var startdate = '1984-01-01';
var enddate   = '1986-12-31';
var dateFilter = ee.Filter.date(startdate,enddate);
// Tutorial uses 25, but 1984-86 Landsat 5 over Barbados is too sparse at that
// threshold. Scene-wide CLOUD_COVER includes ocean clouds far from the island,
// so 60 is still safe — the per-pixel QA_PIXEL mask removes actual cloudy pixels.
...
print('processed Landsat Collection (1984-1986)', landsatFiltered);
print('  scene count', landsatFiltered.size());
print('  L5 scenes only', lt5.filter(dateFilter).size());
...
Map.addLayer(composite, visParamPreProcessed, 'Median Composite 1984-1986');

// --- Step 8: Export ---
Export.image.toDrive({
  image: composite.toFloat(),
  description: 'ToDrive_LandsatComposite_1984_1986',
  fileNamePrefix: 'LandsatComposite_1984_1986',
  region: aoi,
  scale: 30,
  maxPixels: 1e13
});
